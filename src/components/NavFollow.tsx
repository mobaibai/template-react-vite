...
export const NavFollow: React.FC<NavFollowProps<any>> = ({
  ...
  const borderHeight = size === 'small' ? 'border-b-[3px]' : size === 'default' ? 'border-b-[4px]' : 'border-b-[5px]'
      ...
      <div className="nav-list flex flex-wrap relative">
        {
          list && list.length
            ...
            list.map((item: any, index: number) => (
              <div key={nanoid()} ref={navItemRefs[index]} className={`nav-item z-10 mr-[12px] mb-[20px] px-[20px] py-[8px] rounded-md ${fontSize} transition-all duration-200 ${navIndex === index ? `font-bold` : `text-gray-600 hover:text-gray-800 hover:font-bold hover:cursor-pointer`}`} onClick={() => onClickNav(index)} style={{ color: `${navIndex === index ? color : ''}` }}>{item[navItemName]}</div>
            ))
            :
            (
        ...
        <div ref={followItemRef} className={`follow-item absolute z-0 transition-all duration-500 ${followType === 'bg' ? 'bg-gray-200 rounded-md' : `${borderHeight}`}`} style={{ borderColor: color }} />
      </div>
    </div>
  )
}