...
export const AnimationContainer: React.FC<AnimationContainerProps> = ({
  ...
  const animatedStyle = useMemo(() => {
    ...
    if (styles.rotate) {
      const rotateAxis =
        effects.find(e => e.type === 'rotate')?.config?.rotateAxis || 'z'
      transforms.push(
        styles.rotate.to(
          (r: number) => `rotate${rotateAxis.toUpperCase()}(${r}deg)`
        )
      )
    }
  ...
  const transformOrigin = useMemo(() => {
    ...
    return (
      scaleEffect?.config?.transformOrigin ||
      rotateEffect?.config?.transformOrigin ||
      'center'
    )
  }, [effects])
